...
#     then fails with ActiveRecord::InvalidForeignKey (confirmed via
...
# `BranchClaim#latest` now rescues that specific exception and treats it like
# any other lost claim race, so the second case no longer kills the pipeline
# advancer's Thread.
